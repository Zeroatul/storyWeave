...
$stories_sql = "SELECT s.id, s.title, s.genre, s.status, s.synopsis, u.uname AS author_name FROM stories s JOIN user u ON s.user_id = u.id";
$where_clauses = [];
$params = [];
$types = '';
...
if (!empty($where_clauses)) {
    $stories_sql .= " WHERE " . implode(' AND ', $where_clauses);
}

if ($sort === 'recent') {
    $stories_sql .= " ORDER BY s.created_at DESC";
} else {
    $stories_sql .= " ORDER BY s.id DESC";
}
...
<head>
    ...
    <title>Story Library - Story Weave</title>
...
<body>
    ...
    <main class="library-wrapper">
        ...
        <form method="GET" action="story-library.php" class="filter-controls">
        ...
        <div class="story-grid">
            <?php if (!empty($stories)): ?>
                ...
                <?php endforeach; ?>
            <?php else: ?>
                <p>No stories match your criteria. Try adjusting the filters.</p>
            <?php endif; ?>
        </div>
    </main>
